...
def vcs_init(cx):
    ...
    initial_version = "2020-03-06a0.dev0"
...
def repo_test(cx):
    ...
    with cx.prefix(" bimker.dev"):
        cx.run("conda activate bimker.dev && inv -l",
               pty=True)
...
def docs_build(cx):
    ...
    with cx.cd('sphinx'):
        ...
        cx.run("sphinx-apidoc -f --separate --private --ext-autodoc --module-first --maxdepth 1 -o api ../src/bimker")
...
def lint(cx):
    ...
    cx.run("flake8 --output-file=metrics/lint/flake8.txt src/bimker")
...
def complexity(cx):
    ...
    cx.run("lizard -o metrics/code_quality/lizard.csv src/bimker")
    cx.run("lizard -o metrics/code_quality/lizard.html src/bimker")

    # SNIPPET: annoyingly opens the browser

    # make a cute word cloud of the things used
    # cx.run("(cd metrics/code_quality; lizard -EWordCount src/bimker > /dev/null)")
...
def version_which(cx):
    ...
    from bimker import __version__ as proj_version
    print(proj_version)
...
def publish_test(cx):
    ...
    from bimker import __version__ as version
...
#     cx.run(f"cp conda/conda-forge {CONDA_FORGE_RECIPE_PATH}/bimker")
